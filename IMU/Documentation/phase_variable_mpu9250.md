# IMU Phase Variable Pipeline — MPU9250

Real-time thigh-angle phase variable estimation (Raspberry Pi + MPU9250) with MATLAB verification plots.

---

## 1. Python — `phase_variable_mpu_9250.py`

Reads accel/gyro from the MPU9250 over I2C, fuses pitch with a Kalman filter, computes a centered pitch + filtered gyro rate, and maps them through `atan2` into a phase variable φ ∈ [0, 1] with a monotonicity safeguard against noise-induced backward steps.
...
## 2. MATLAB — `plot_phase_var.m`

Loads the CSV produced by the Python logger and plots φ vs time, pitch, and filtered gyro rate, plus a phase-colored scatter portrait. Prints range/mean stats to sanity-check that φ sweeps 0→1 each gait cycle.
...
**Bottom panel (pitch centered, deg):** smooth oscillation crossing the ±20° reference lines, confirming pitch_centered_deg is driving the phase wraps at the expected sawtooth resets.
