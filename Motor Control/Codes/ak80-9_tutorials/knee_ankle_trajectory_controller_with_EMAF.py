...
# Section 3.11. The SAME two numbers used by
# knee_ankle_trajectory_controller_blpf_filter.py's Butterworth filter are
# reused here as the starting point -- not because the two filters are
# identical (they are not -- see §3.11 Part 6), but because both filters
# are damping the SAME physical problem: the joint's own ~16 Hz mechanical
# resonance, sitting INSIDE the same Kd feedback path.
#
...
#
# TUNING PROCEDURE (do not skip this -- identical in spirit to §3.10's):
#   1. Run at 140 Hz first and confirm the trajectory still tracks as well
#      as it did with no filter at all (knee_ankle_trajectory_controller_
#      nofilter.py). This is your stable baseline.
#   2. Step DOWN: 140 -> 100 -> 80 -> 60, one run at a time, one joint at
#      a time.
